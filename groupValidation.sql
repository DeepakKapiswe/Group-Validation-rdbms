...
DROP TABLE IF EXISTS [aux_currentGroupMembersRollNosGId];
...
DROP TABLE IF EXISTS [aux_invokeMemberAgreementCheck];
...
DROP TABLE IF EXISTS [input_generateReminderMsgs];
...
DROP TRIGGER IF EXISTS trigger_generateReminderMsgs;
...
CREATE TABLE [aux_currentGroupMembersRollNosGId]
(
    [groupId] TEXT NOT NULL,
    [senderRollNo] TEXT  NOT NULL,
...
(
    [memberRollNo] TEXT NOT NULL,
    CONSTRAINT [PKC_aux_maybeMemberRollNos] PRIMARY KEY ([memberRollNo])
);

CREATE TABLE [aux_invokeMemberAgreementCheck]
(
...
CREATE TABLE [input_generateReminderMsgs]
(
    [val] TEXT PRIMARY KEY
);
...
when (select length(new.msg)) > 0
  begin
    insert into aux_maybeMemberRollNos select substr(new.msg,1,(select instr(new.msg,' ')-1));
    insert into aux_maybeRollNos select substr(new.msg,(select instr(new.msg,' ')+1));
  end;
...
begin
    ...
    delete from aux_currentGroupMembersRollNosGId;
    delete from aux_invokeMemberAgreementCheck;
    ...
    insert into aux_currentGroupMembersRollNosGId select groupId,senderRollNo from aux_maybeGroups where groupId=new.groupId;

    insert into aux_invokeMemberAgreementCheck values (1);
end;

CREATE TRIGGER trigger_insertCurrentGroupMemberGIds after insert on aux_currentGroupMembersRollNos
begin
    insert into aux_currentGroupMembersRollNosGId select groupId,senderRollNo from aux_maybeGroupMembers where senderRollNo=new.memberRollNo and memberRollNo=new.senderRollNo;
end;

CREATE TRIGGER trigger_updateGroupMemberAgreementStatus after insert on aux_maybeGroupMembers
when (select count(*) from aux_maybeGroupMembers where agreementStatus='No' and senderRollNo=new.memberRollNo and memberRollNo=new.senderRollNo) = 1
begin
...
create trigger trigger_checkIfGroupMembersAgree after insert on aux_invokeMemberAgreementCheck
when (select count(agreementStatus) from aux_maybeGroupMembers where groupId in (select groupId from aux_currentGroupMembersRollNosGId) and agreementStatus='No') = 0
begin
    insert into aux_agreedGroups  select * from ((select group_concat(groupId,'') from aux_currentGroupMembersRollNosGId) inner join aux_currentGroupMembersRollNosGId);
end;
...
begin
    ...
    insert into output_finalValidGroups select * from aux_agreedGroups where finalGId not in (select finalGId from output_conflictingGroups) and finalGId not in (select finalGId from output_lengthExceededGroups);

    delete from input_generateGroupReports;
end;

CREATE TRIGGER trigger_generateReminderMsgs after insert on input_generateReminderMsgs
begin
  delete from output_reminderMsgs;
  insert into output_reminderMsgs select * from (select senderEmailId,'No application got in favour of -> '||sroll||' from '||mrolls from aux_maybeGroups inner join (select senderRollNo as sroll,group_concat(memberRollNo) as mrolls from aux_maybeGroupMembers where groupid in (select groupid from output_nonAgreedGroups) and agreementstatus='No' group by groupid) where sroll=senderRollno);
end;
...
insert into input_userApplications values('5273 64336 ','ssaetgt@gmail.com');

insert into input_userApplications values('41 50 ','ewwwtgt@gmail.com');
...
insert into input_generateReminderMsgs values (1);

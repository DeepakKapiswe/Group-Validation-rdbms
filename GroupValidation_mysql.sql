...
FOR EACH ROW
  begin
    ...
    insert into aux_userApplicationInfo (senderRollNo,senderEmailId,timestamp,validationResult) values ((select substr(new.msg,1,(select instr(new.msg,' ')-1))),new.senderEmail,datetime,'valid');
  end;

CREATE TRIGGER trigger_getMaybeRollNos after insert on aux_maybeRollNos
FOR EACH ROW
  begin
    if (select length(new.remainingMemberRollString)) > 0
      then
...
begin
    ...
    insert into aux_maybeGroupMembers (groupId,senderRollNo,memberRollNo) select * from (select groupId,senderRollNo from aux_maybeGroups where groupId =new.groupId) left join aux_maybeMemberRollNos;
